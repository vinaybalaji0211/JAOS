...
PostgreSQL Memory Transaction

Provides native PostgreSQL transactions for atomic memory writes.
"""
...
class PostgreSQLTransaction(MemoryTransaction, MemoryWriter):
    """
    Native PostgreSQL transaction for atomic memory write operations.

    The transaction uses the PostgreSQLStore connection and holds the store
    lock for the complete transaction lifetime.
    """
    ...
    def __init__(
        ...
        """
        Initialize a PostgreSQL transaction.

        Args:
            store:
                PostgreSQL store that owns the database connection.
        """
        self._store = store
        self._active = False
    ...
    def __enter__(self) -> Self:
        """
        Begin a native PostgreSQL transaction.
        """
        if self._active:
            raise RuntimeError(
