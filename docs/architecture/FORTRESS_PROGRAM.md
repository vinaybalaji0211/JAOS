...
### 7.21 FORTRESS-06D Memory Retirement

Date: 2026-08-31. Under ADR-0013, the four remaining configured legacy
Executive Memory importers were retired from executable collection and retained
as byte-identical, non-Python `*.py.legacy` archives:

| Retired configured path | Collected tests | Preserved archive | SHA-256 |
|---|---:|---|---|
...
| Total | 30 | Four byte-identical archives | Verified |

No canonical `WorkingMemory`, `MemoryManager`, or `MemoryRegistry` replacement
was created. Canonical persistent Memory remains owned and independently tested
through `MemoryStore`/`SQLiteStore`. Transient request/context and
mission/plan/decision/result responsibilities remain preserved for explicitly
approved future owners; durable forms remain under FORTRESS-08 where
applicable; working-memory health remains deferred to FORTRESS-10; Experience
Memory remains a separate future platform; and RAA-009 continues to govern
deferred `MemoryContextSource`/`MemorySearchEngine` coupling.

The existing collection-containment authority now proves the four executable
paths are absent, all four archives preserve their exact payloads and 30 source
tests, the archives are not Python-importable or pytest-collectable, no
`__init__.py` exists under `legacy_quarantine/`, zero configured legacy Memory
importers remain, and the only remaining configured `executive_brain` importers
are:

- `tests/tests/ai/test_ollama_provider.py`; and
- `tests/tests/ai/test_openai_provider.py`.

Mechanical AST/import analysis verified these achieved changes:

- configured legacy-facing files: 19 -> 15; and
- configured `executive_brain` importers: 6 -> 2.

The exact 15 remaining configured legacy-facing files are partitioned as:

- two provider tests: `tests/tests/ai/test_ollama_provider.py` and
  `tests/tests/ai/test_openai_provider.py`;
...
- three platform/core/kernel/config files:
...
The four retired tests and their legacy implementations mutate only in-memory
dictionaries, dataclass fields, `ServiceContainer`, `RuntimeContext`, and
`EventBus` state. They invoke no filesystem writer or canonical `SQLiteStore`;
no `data/*.json`, SQLite, repository, or runtime-data migration occurred.
Legacy `executive_brain.memory.MemoryManager`, `MemoryRegistry`, and
`WorkingMemory` production sources remain importable for later approved
production-source/compatibility disposition, but remain absent from
`run_jaos.py`, `PlatformComposition`, canonical Executive, and canonical
Conversation production paths.

Verified successful gates used repository Python 3.14.6,
`PYTHONDONTWRITEBYTECODE=1`, `-B`, `-p no:cacheprovider`, unique external
`--basetemp` paths, and the established temporary Windows ACL runner shim:

| Gate | Result |
|---|---|
| Four retired source files before quarantine | 30 collected |
| Focused containment/Memory stores/transactions/runtime binding/composition/import boundary | 243 passed |
...
| Repository-root collection | 1,808 collected |
| Ruff on the changed Python containment authority | All checks passed |

Collection reconciles exactly as `1,836 - 30 + 2 = 1,808`: all 30 retired
source tests were previously collected and two new containment tests were
added. Preliminary direct and incompletely scoped shim runs encountered the
documented Python 3.14 Windows `0o700` temporary-directory ACL failure. After
the established shim covered the complete external pytest temp tree, the first
valid focused run exposed one stale D2C containment assertion; that historical
assertion was updated for ADR-0013 and the complete ladder above passed.

No production code changed, no canonical Memory contract changed, no runtime
data migrated, and no F08, F10, RAA-009, Context, Experience Memory, provider,
satellite, production-source, or Phase 8 capability work began.

FORTRESS-06D Memory retirement — IMPLEMENTED AND VERIFIED.
It completes neither F06D nor FORTRESS-06. RAA-003 remains OPEN, RAA-007 remains
RESOLVED WITH EVIDENCE, RAA-009 remains OPEN — DEFERRED, F07/F08/F10 remain NOT
...
### 7.22 FORTRESS-06D Provider Adjudication Governance — ADR-0014

Date: 2026-08-31. The read-only provider adjudication is COMPLETE. It identified
the final two configured `executive_brain` importers and 20 source tests, all 20
of which are pytest-collected:

| Configured file | Source tests | Collected cases | Evidence character |
|---|---:|---:|---|
| `tests/tests/ai/test_ollama_provider.py` | 9 | 9 | Offline/mock-based legacy Ollama adapter evidence |
| `tests/tests/ai/test_openai_provider.py` | 11 | 11 | Offline/mock-based legacy OpenAI adapter evidence |
| Total | 20 | 20 | Neither file proves real provider integration |

Both files directly exercise unreachable `executive_brain` shadow adapters.
ADR-0014 records the Founder-approved PORT + QUARANTINE disposition. Before a
later controlled retirement, exactly three current provider-neutral invariants
must receive configured canonical evidence:
...
2. `ProviderManager.generate()` rejects invalid or non-`AIRequest` input before
   provider execution.
3. Provider generation failure is normalized as `ProviderManagerError`, with
   truthful canonical failure metrics and state.

Canonical production already supports these invariants; this governance task
does not implement their tests or modify production. No OpenAI- or
Ollama-specific behavior is to be ported in FORTRESS-06.

Provider independence remains mandatory. Current runtime authority is
`PlatformComposition` -> `ProviderManager`/`AIManager` -> canonical provider
abstractions -> deterministic `MockProvider`. OpenAI is an initial concrete
reference-provider candidate for FORTRESS-09 only; it is not architectural
authority, a permanent dependency, the sole supported provider, or a condition
for local/offline JAOS operation. Ollama remains optional and is not required
for FORTRESS-09 certification.

FORTRESS-09 remains NOT STARTED. It retains later ownership of approved
concrete-provider adapters and resilience concerns, including timeout
enforcement, retries, circuit breaking, fallback, health/reachability,
unavailable-provider and malformed-response handling, auth and rate-limit
failures, secret/config integration, graceful degradation, telemetry, model
...
### 7.26 FORTRESS-06E Communication Production-Root Quarantine Pilot

Date: 2026-09-01. The controlled F06E pilot quarantined the six tracked Python
sources that comprised the live `communication/` root. Static caller analysis
verified zero canonical production callers, zero legacy production callers,
and zero configured-test callers. No active dynamic loader can reach
`communication.*`. Seven excluded flat historical tests continue to reference
the former imports; they remain unchanged outside configured `tests/tests`
certification as later F06G/F06H compatibility debt.

The exact source-preservation evidence is:

| Retired production source | Preserved production archive | SHA-256 | Git blob |
|---|---|---|---|
| `communication/calendar_manager.py` | `legacy_quarantine/production/communication/calendar_manager.py.legacy` | `de550bf3ddb8ea4c7c9be56e492f54c54dfa5280a7e8ef853def40eea8894911` | `807089aa2bc86cf92f42ae65fa8ececddc13db98` |
| `communication/communication_hub.py` | `legacy_quarantine/production/communication/communication_hub.py.legacy` | `d34c4f6b33410c69498493675fef02d59ae88e7bee366c97d6311c0295eca02b` | `65bb8f303128b6847fa2ac9913af5d627ad82ec1` |
| `communication/contacts_manager.py` | `legacy_quarantine/production/communication/contacts_manager.py.legacy` | `2f0c86a51200d40513d3827def63c464c8f3fc10d2cce433a761a999932bb565` | `69f189614e5c5fbf707561752b99fec3fd9a154f` |
| `communication/conversation_manager.py` | `legacy_quarantine/production/communication/conversation_manager.py.legacy` | `8bace0ec95836012cf3cf7fceb2fae929068ca975c010502a33e51a05cb0e72b` | `3dacf67cda7110b7b92c8b76e9141a63ee5470c4` |
| `communication/email_manager.py` | `legacy_quarantine/production/communication/email_manager.py.legacy` | `997d2c4ea5cfb8fd82194d86158a07b5b8d593dcc22cb5f0fec42f595500d08b` | `824e1eccc45973d45aad7988397e86df211b6dd7` |
| `communication/meeting_assistant.py` | `legacy_quarantine/production/communication/meeting_assistant.py.legacy` | `e8ff0e6b877b3c4546c4c6810cb418c41b0210b5896956fa60de8608f35bf542` | `c36d257ebcf8a5b91ca4f4c96b43655581a7789a` |
| Total | Six byte- and Git-blob-identical `*.py.legacy` archives | Verified | Verified |

The live `communication/` production root is absent. The archive uses no
Python import suffix, contains no `__init__.py`, is non-collectable, and is not
imported by repository Python source. No forwarding wrapper, compatibility
stub, replacement Communication capability, or other canonical behavior was
introduced. The canonical `run_jaos.py` static closure remains disjoint from
the retired root, and `jaos/` plus `jaos_platform/` remain unchanged.

Static inspection verified that the six sources implement no persistent
writer, runtime-data writer, network request, subprocess, process or thread
creation, desktop/application/service control, or other external-effect
capability requiring preservation. No runtime-data migration occurred. The
retained `tests/tests/platform/test_config_containment.py` remains unchanged
with nine source definitions and eleven collected cases; this pilot does not
satisfy its F06E/F06F retirement gate.

The existing collection-containment authority gained exactly two configured
cases: production-archive fidelity and communication caller containment.
Verified repository-Python gates used `PYTHONDONTWRITEBYTECODE=1`, `-B`,
...
Collection reconciles exactly as `1,761 + 2 = 1,763`; the production-source
move retired no configured tests.
...
### 7.27 FORTRESS-06E Development/Infrastructure/PC-Control Production Quarantine

Date: 2026-09-05. Status: IMPLEMENTED AND VERIFIED.

This controlled production-source slice retired exactly 24 Python sources:
`development/` 7, `infrastructure/` 9, and `pc_control/` 8. The pre-move
baseline contained exactly the tracked HEAD sources, including the three empty
package initializers; manually removed caches were verified absent. Every
payload was moved byte-identically into
`legacy_quarantine/production/<original-relative-path>.legacy`.

The [quarantine manifest](FORTRESS_06_LEGACY_QUARANTINE_MANIFEST.md#21-fortress-06e-developmentinfrastructurepc-control-production-quarantine)
records all 24 exact original/archive mappings, working-byte SHA-256 values,
and Git blobs. HEAD checkout bytes account for the repository's
`core.autocrlf=true` policy; no archived payload was edited or normalized.
Original and archive paths produce the same HEAD Git blob, preserving
R100-equivalent identity and reversible history.

All three live roots are absent, with no recreated caches. The archives are
non-importable/non-collectable `.py.legacy` files, contain no importable
quarantine `__init__.py`, and have no repository Python importer. No wrapper,
stub, alias, forwarding module, or replacement capability was added.

| Root | Sources | Canonical callers | Legacy production callers | Configured-test callers | Active dynamic paths | Excluded flat files / import statements |
|---|---:|---:|---:|---:|---:|---:|
| `development` | 7 | 0 | 0 | 0 | 0 | 7 / 12 |
| `infrastructure` | 9 | 0 | 0 | 0 | 0 | 9 / 16 |
| `pc_control` | 8 | 0 | 0 | 0 | 0 | 8 / 14 |
| Total | 24 | 0 | 0 | 0 | 0 | 24 / 42 |

Static inspection reconfirmed LOW-RISK IN-MEMORY disposition and no
root-owned writer/external-effect capability requiring preservation. Stored
commands, URLs, application names, and paths were not executed. Shared logger
and optional runtime-event use did not establish canonical callers or a
root-owned persistent writer. The exact 24 excluded flat files / 42 direct
import statements remain pinned in containment evidence as F06G/F06H debt.

Exactly two grouped cases extend the existing collection-containment file:
archive fidelity and caller/boundary preservation. The three prior live-file
requirements for DevelopmentWorkspaceManager, AIProviderManager, and
ApplicationManager now require their exact archives; unrelated live satellite,
core/kernel, and archive assertions remain intact.

The canonical import-boundary test's stale communication classification was
narrowly corrected. Exact path sets now place communication, development,
infrastructure, and pc_control production archives in E: D=12, E=7, total 33.
No unrelated classification, forbidden-module guard, exact-set comparison,
duplicate protection, or canonical architecture coverage was removed.

`jaos/`, `jaos_platform/`, `run_jaos.py`, BasePlatformService, and
PlatformContract are unchanged. The canonical launcher closure remains
statically disjoint; this does not certify live runtime readiness.
`tests/tests/platform/test_config_containment.py` retains SHA-256
`d862bd601301ae7bfc85aa16a5cc9f31e5f77b23bc54e84689a4276a5a2a447c`,
9 source definitions, and 11 cases. Configured legacy-facing remains 1 and
configured `executive_brain` importers remain 0; its F06E/F06F writer gate is
unchanged.
...
The authorized task comprises 52 logical paths: 24 retired sources, 24
archives, two test files, and these two architecture evidence documents.
...
The Founder dispositions for main.py and BasePlatformService/PlatformContract
remain unresolved. Step 8 remains blocked, Fortress certification has not
started, and major Phase 8 expansion remains paused. Nothing was staged,
committed, or pushed.
...
### 7.30 FORTRESS-06E Kernel Production-Root Quarantine
...
The exact 12-source path/SHA/blob/size map and 11-script debt inventory are
recorded in manifest section 24. All archive payloads remain unchanged.
